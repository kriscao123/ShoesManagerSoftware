...
public class GenerateAllPdfForOneCN {
	private LuongDao luongDao;
	private List<Luong> dsLuong;
	private NhanVienDao nvDao;
	private List<NhanVien> dsNV;

	public GenerateAllPdfForOneCN(String tenNV, String maNV) {
		...
		try {
			...
			PdfWriter write = PdfWriter.getInstance(doc,
					new FileOutputStream("G:/Desktop/hoadon/" +"DS_" + maNV.trim() +"_"+ tenNV.trim() + ".pdf"));

			doc.open();
			Paragraph p1 = new Paragraph("Hữu Nghị - Uy Tín, Thái Độ, Chất Lượng", f2);
			p1.setAlignment(Paragraph.ALIGN_CENTER);
			Paragraph p2 = new Paragraph("PHIẾU XUẤT DANH SÁCH LƯƠNG CÔNG NHÂN", f2);
			...
			String s1 = String.format("%1s" + "" + "%1s" + "Nhân viên lập:%3s" + tenNV, "", "", "");
...
//			doc.add(new Paragraph(String.format("Địa chỉ:%7s" + kh.getDiaChi(), ""), f1));
			...
			int i = 1;
			for (Luong l : dsLuong) {
				if(l.getNhanVien().getMaNhanVien().equals(maNV)) {
					...
					for(NhanVien x : dsNV) {
						if(l.getNhanVien().getMaNhanVien().equals(x.getMaNhanVien())) {
							c11 = new PdfPCell(new Paragraph(x.getTenNhanVien(), f1));
						}
					}
			...
			doc.add(new Paragraph("\n\n\n"));
			doc.add(new Paragraph(String.format("%8s %-30s %110s %-30s", "", "", "", tenNV), f1));
			doc.add(new Paragraph("\n"));
