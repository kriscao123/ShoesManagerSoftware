...
public class GenerateAllPdfCN {
	...
	public GenerateAllPdfCN(String tenKT) {
		...
		try {
			...
			PdfWriter write = PdfWriter.getInstance(doc,
					new FileOutputStream("G:/Desktop/hoadon/" +"DanhSachPhieuLuongCN"+ ".pdf"));
...
//			doc.add(new Paragraph(String.format("Địa chỉ:%7s" + kh.getDiaChi(), ""), f1));
			...
			int i = 1;
			for (Luong l : dsLuong) {
				if(l.getNhanVien().getMaNhanVien().charAt(0) == 'C') {
