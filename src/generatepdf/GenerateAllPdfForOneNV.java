...
public class GenerateAllPdfForOneNV {
	private LuongDao luongDao;
	private List<Luong> dsLuong;
	private NhanVienDao nvDao;
	private List<NhanVien> dsNV;

	public GenerateAllPdfForOneNV(String tenNV, String maNV) {
		...
		Document doc = new Document();
			try {
				...
				PdfWriter write = PdfWriter.getInstance(doc,
						new FileOutputStream("G:/Desktop/hoadon/" +"DS_"+maNV.trim()+ "_"+ noSpaceStr.trim() +".pdf"));

				doc.open();
				Paragraph p1 = new Paragraph("Hữu Nghị - Uy Tín, Thái Độ, Chất Lượng", f2);
				p1.setAlignment(Paragraph.ALIGN_CENTER);
				Paragraph p2 = new Paragraph("PHIẾU XUẤT DANH SÁCH LƯƠNG CHO MỘT NHÂN VIÊN", f2);
